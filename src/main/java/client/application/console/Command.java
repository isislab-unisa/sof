...
import client.application.SCUDShellClient;

import com.jcraft.jsch.JSchException;

import core.engine.hadoop.sshclient.connection.HadoopFileSystemManager;
...
import core.engine.hadoop.sshclient.utils.simulation.Simulations;
...
public enum Command implements Prompt
{
		...
		public Object exec(Console c, String[] params,String stringPrompt)
		{
			...
			if(params.length > 1)
			{
				for(int i=0; i < params.length ;i++)
				{
					...
					{
						if(Enum.valueOf(Command.class, params[i].toUpperCase()).equals(Command.SUBMIT))
						{
							c.printf("usage: submit TO WRITE\n");

						}else
							if(Enum.valueOf(Command.class, params[i].toUpperCase()).equals(Command.CREATE))
							{
								c.printf("usage: create\n");
							}else
								if(Enum.valueOf(Command.class, params[i].toUpperCase()).equals(Command.CREATESIMULATION))
								{
									c.printf("usage: createsimulation <MODEL>[(M)ASON-(N)ETLOGO-(G)ENERIC] <SIM-NAME> <INPUT.xml>"
											+ " <Output.xml> <DESCRIPTION-SIM> <SIMULATION-EXECUTABLE-MODEL>\n");

								}else
									if(Enum.valueOf(Command.class, params[i].toUpperCase()).equals(Command.CREATESIMULATIONLOOP))
									{
										c.printf("usage: createsimulationloop TO WRITE\n");
									}else
										if(Enum.valueOf(Command.class, params[i].toUpperCase()).equals(Command.GETSIMULATIONS))
										{
											c.printf("usage: getsimulations TO WRITE\n");
										}else
											if(Enum.valueOf(Command.class, params[i].toUpperCase()).equals(Command.GETSIMULATION))
											{
												c.printf("usage: getsimulation TO WRITE\n");
											}
											else
												if(Enum.valueOf(Command.class, params[i].toUpperCase()).equals(Command.PRINTRESULT))
												{
													c.printf("usage: printresult TO WRITE\n");
												}
												else
													if(Enum.valueOf(Command.class, params[i].toUpperCase()).equals(Command.GETRESULT))
													{
														c.printf("usage: getresult TO WRITE\n");
													}
					}
	...
	CREATE(new Action()
	{
		...
		public Object exec(Console c, String[] params,String stringPrompt)
		{
			c.printf("To create a new simulation use createsimulation or createsimulationloop. (help for more information)\n");
			return null;
		...
		public Object exec(Console c, String[] params,String stringPrompt)
		{
			...
			if(parsedParams.length == 6)
			{
				...
				c.printf("usage: MODEL[MASON-NETLOGO-GENERIC] SIM-NAME[String] INPUT.xml[String absolutely]"
						+ " Output.xml[String absolutely path] DESCRIPTION-SIM[String] NOTE-SIM[String] SIMULATION-EXECUTABLE-MODEL[String absolutely path]\n");
				return null;
			}
		}
	}),
	...
	LIST(new Action()
	{
		...
		public Object exec(Console c, String[] params,String stringPrompt)
		{
			...
			for(int i=1; i<=sims.getSimulations().size(); i++){
				...
				c.printf(i+") name: "+s.getName()+" state: "+s.getState()+" time: "+s.getCreationTime()+" id: "+s.getId()+"\n");
			}
			return null;
		}
	}),
		...
		public Object exec(Console c, String[] params,String stringPrompt)
		{
			/*if(params == null || params.length < 1 )
			{
				...
				try {
					...
					if(!PromptCommands.outputList.isEmpty())
